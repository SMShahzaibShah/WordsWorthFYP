...
export default function book({ navigation, route }) {
  ...
  const outputCollections = async () => {
    // console.log(Coll);
    var allcoll = await AsyncStorage.getItem("Collections");
    if (allcoll != null) {
      allcoll = JSON.parse(allcoll);
      // console.log("book", allcoll);
      //  if (allcoll.length > 0) {
      setColl(allcoll);
      // }
    }
          ...
          <>
            {Coll.length > 0 ? (
              <TouchableOpacity
                ...
                onPress={() => {
                  //  console.log(item);
                  navigation.navigate("bookCollections", {
                    collectionInfo: item,
                  });
                }}
              >
  ...
  const SaveData = async (item) => {
    await AsyncStorage.getItem("Collections").then((favs) => {
